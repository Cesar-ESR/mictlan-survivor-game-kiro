/**
 * Game constants used throughout the Mictlán Survivor game.
 * Contains all base values for player, map, waves, enemies, weapons, orbs, and scaling.
 */
export const GAME_CONSTANTS = {
  // Player
  PLAYER_BASE_SPEED: 200,           // px/s
  PLAYER_BASE_HP: 100,
  MAX_LEVEL: 20,

  // Map
  MAP_WIDTH: 3200,
  MAP_HEIGHT: 3200,

  // Waves
  WAVE_DURATION: 30,                // seconds
  WAVE_TRANSITION_TIME: 2,          // seconds
  BASE_SPAWN_INTERVAL: 2,           // seconds

  // Enemies
  DEFAULT_MAX_ENEMIES: 100,
  ENEMY_DESPAWN_DISTANCE: 1500,     // px

  // Spawn positioning
  SPAWN_MIN_DISTANCE_FROM_EDGE: 50,
  SPAWN_MAX_DISTANCE_FROM_EDGE: 300,

  // Weapon
  WEAPON_BASE_DAMAGE: 10,
  WEAPON_BASE_FIRE_RATE: 1000,      // ms
  WEAPON_RANGE: 800,                // px
  PROJECTILE_MAX_DISTANCE: 1000,    // px

  // Combat
  CONTACT_DAMAGE_COOLDOWN: 1000,    // ms

  // Orbs
  ORB_ATTRACT_RADIUS: 100,          // px
  ORB_ATTRACT_SPEED: 400,           // px/s
  ORB_LIFETIME: 30,                 // seconds
  MAX_ORBS: 200,

  // XP
  XP_THRESHOLD_FORMULA: (level: number): number => level * 10 + 5,

  // Difficulty scaling
  SPAWN_INTERVAL_DECAY: 0.9,
  MIN_SPAWN_INTERVAL: 0.5,          // seconds (floor)
  HP_SCALING_BASE: 1.15,
  MAX_HP_MULTIPLIER: 5,             // ceiling
  SPEED_SCALING_BASE: 1.05,
  MAX_SPEED_MULTIPLIER: 2,          // ceiling

  // Calavera Llameante
  EXPLOSION_RADIUS: 100,            // px
  EXPLOSION_DAMAGE: 15,
} as const;
